\section{Topology}
\label{chap:topology}
I widget lato \textit{mobility} sono impiegati per mostrare i dati provenienti dalle macchine la cui struttura si articola in \textit{linee, banchi e stazioni; queste ultime sono la parte del macchinario volta a effettuare i test sui vari componenti che attraversano la linea.
...
\end{lstlisting}
Le classi che attualmente la implementano sono \verb|AllRefusedDataSource| e\\ \verb|RefusedTypeDataSource|, le quali andranno a richiamare i metodi necessari per la richiesta della lista dei codici di linee, banchi o stazioni, presente nella repository collegata a un database MySQL.
