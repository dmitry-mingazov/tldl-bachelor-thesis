...
\begin{center}
  ...
  \caption{Material Icon Theme}\label{fig:VisualStudioCode}
\end{center}
\end{figure}
\item
...
\item
\begin{minipage}{0.1\textwidth}
\includegraphics[width=2cm] {images/TSLint.png}
\end{minipage}
Estensione per la correzione automatica.
\end{itemize}
...
% INSERIRE IMMAGINE POSTMAN RICHIESTA A DATI DI UN WIDGET
...
L'applicativo backend del progetto discusso nella tesi è stato sviluppato utilizzando ASP.NET Core 3.1.\\
ASP.NET Core è una versione open-source e cross-platform di ASP.NET, un framework per costruire web apps all'interno della piattaforma .NET.
\subsection{.NET}
.NET è una piattaforma di sviluppo creata per costruire differenti tipi di applicazioni, utilizzando il C#, F# o Visual Basic.
\begin{itemize}
